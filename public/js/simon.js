...
function releaseClick() {
	if (errorFlag === 0) {
		...
		if (locationFlag !== -1) {
			...
			if (locationFlag === sequence[round]) {
				//correct input
				if (round + 1 === currentNum) {
					//last round
					phase = 3;
					if (round === 19) {
						//won
				...
				if (mode === 0) {
					setTimeout(function () {
						playSequence();
					}, 1000); //play sequence again after 1 second
				} else {
					resetGame();
					setTimeout(function () {
						clickStart();
...
function playDelay(n, lastFlag) {
	allDelays[allDelays.length] = setTimeout(function () {
		playSound(sequence[n]);toggleBackgroundColor(sequence[n]);
	}, n * 1000);
	allDelays[allDelays.length] = setTimeout(function () {
		stopSound();toggleBackgroundColor(sequence[n]);
	}, n * 1000 + 800);
	if (lastFlag === 1) {
		allDelays[allDelays.length] = setTimeout(function () {
			phase = 2;updateCounter(currentNum);
		}, (n + 1) * 1000);
		errorFlag = 0;
	}
}
